<script>
  ...
  // check for teams outside of the minimumPlayers/maximumPlayers allowed team size
  $: badSizedTeams = $data.teams.filter((t) => {
    const players = t.players;
    if (!t.players.length) return false;
    return (
      players.length < $data.minimumPlayers ||
      players.length > $data.maximumPlayers
    );
  });
...
{#each [currentSession] as s (s)}
  <div class="player-grid">
    ...
    {:else if badSizedTeams.length}
      <div class="all-players-managed">
        <svg
          class="icon theme-caution-color"
          width="57"
          height="57"
          viewBox="0 0 57 57"
          fill="none"
