<script>
  ...
  $: outsideOfMinMax =
    players.length &&
    (($data.minimumPlayers && players.length < $data.minimumPlayers) ||
      ($data.maximumPlayers && players.length > $data.maximumPlayers));
</script>

<div
  class="player-item is-header theme-caution-color"
  class:theme-caution-color={outsideOfMinMax}
  id={team.internalId || team.id}
>
  <h2
  ...
  <p class="player-card-summary">
    ...
    {#if outsideOfMinMax}
      {#if $data.minimumPlayers && (!$data.maximumPlayers || $data.minimumPlayers === $data.maximumPlayers)}
        <em>({$data.minimumPlayers} required)</em>
      {:else if $data.minimumPlayers && $data.maximumPlayers}
        <em>({$data.minimumPlayers} to {$data.maximumPlayers} required)</em>
      {:else if $data.maximumPlayers}
        <em>({$data.maximumPlayers} maximum)</em>
      {/if}
    {/if}
  </p>
  <div class="player-card-actions">
    ...
    >
      <svg
        class="icon"
        viewBox="0 0 15.679 21.169"
        id="trash"
        xmlns="http://www.w3.org/2000/svg"
        ><path
          d="M15.67 4.446l-.057-.215-.409-1.545a.876.876 0 00-.692-.588l-.034-.005a51.6 51.6 0 00-3.453-.33l-.04.002a.638.638 0 01-.614-.46C10.234.993 9.978.311 9.663.184A1.438 1.438 0 009.448.12C9.39.11 9.331.103 9.272.095a10.745 10.745 0 00-2.867 0C6.348.103 6.288.11 6.23.12a1.225 1.225 0 00-.213.065C5.709.308 5.459.98 5.32 1.28a.634.634 0 01-.62.485c-.011 0-.021 0-.032-.002-1.146.071-2.28.18-3.404.325l-.142.018a.876.876 0 00-.648.581L.065 4.231l-.057.215a.904.904 0 00.874 1.027h13.915a.905.905 0 00.873-1.027zm-1.695 2.69H1.704a.906.906 0 00-.812.859l.02.232 1.005 12.18a.88.88 0 00.869.762h10.107c.44 0 .812-.326.869-.762l1.007-12.18.019-.232a.908.908 0 00-.813-.859z"
          fill="currentColor"
        /></svg
      >
    </a>
